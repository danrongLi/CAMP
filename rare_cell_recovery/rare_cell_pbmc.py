...
PRETTY_METRIC_LABELS = {
    ...
    #"best_f1": "Best Metacell Purity-Recall F1",
...
def run_pbmc_rare_cell_experiment(
    ...
    raw_all_csv = os.path.join(output_root, "PBMC_rare_cell_ALL_raw.csv")
    summary_all_csv = os.path.join(output_root, "PBMC_rare_cell_ALL_summary.csv")
    ...
    legend_png = os.path.join(plots_dir, "PBMC_rare_cell_legend.png")
    ...
    for rare_thr in rare_thresholds:
        ...
        for metric in metrics_to_plot:
            out_png_m = os.path.join(
                thr_dir,
                f"PBMC_{metric}_vs_nmetacells_rare{str(rare_thr).replace('.', 'p')}.png"
            )
            plot_metric(
    ...
    combined_legend_png = os.path.join(combined_boxplot_dir, "PBMC_grouped_boxplot_legend.png")
    ...
    for rare_thr in rare_thresholds:
        ...
        for metric in [
            ...
            out_png = os.path.join(
                thr_dir,
                f"PBMC_grouped_celltype_methods_{metric}_rare{str(rare_thr).replace('.', 'p')}.png"
            )
    ...
    for rare_thr in rare_thresholds:
        ...
        for method in methods_present:
            out_png = os.path.join(
                thr_dir,
                f"PBMC_{method}_assignment_heatmap_rare{str(rare_thr).replace('.', 'p')}.png"
            )
