...
def resolve_preprocessed_h5ad(
    ...
    candidates = [
        os.path.join(preprocessed_dir, f"{dataset_name}_hvg_{variation_value}_prep.h5ad"),
        os.path.join(preprocessed_dir, f"{dataset_name}_camp1_hvg_{variation_value}_prep.h5ad"),
        os.path.join(preprocessed_dir, f"{dataset_name}_camp2_hvg_{variation_value}_prep.h5ad"),
        os.path.join(preprocessed_dir, f"{dataset_name}_camp3_hvg_{variation_value}_prep.h5ad"),
        os.path.join(preprocessed_dir, f"{dataset_name}_camp4_hvg_{variation_value}_prep.h5ad"),
    ]
    for p in candidates:
        if os.path.exists(p):
    ...
    raise FileNotFoundError(
        f"Could not find preprocessed h5ad for {dataset_name} hvg={variation_value} "
...
def make_lineplots_for_downstream(
    ...
    save_method_legend_png(
        os.path.join(out_dir, f"{dataset_name}_hvg_method_legend.png"),
    ...
    save_value_legend_png(
        os.path.join(out_dir, f"{dataset_name}_hvg_value_legend.png"),
        values=[int(v) for v in values],
        dpi=dpi,
    )

    save_overlay_style_legend_png(
        os.path.join(out_dir, f"{dataset_name}_hvg_overlay_style_legend.png"),
        dpi=dpi,
    )

    save_near_fixed_metacell_style_legend_png(
        os.path.join(out_dir, f"{dataset_name}_hvg_near_fixed_metacell_style_legend.png"),
    ...
    plot_balacc_overlay_all_values(
        ...
        out_png=os.path.join(overlay_dir, f"{dataset_name}_hvg_bal_acc_top10_overlay.png"),
        dpi=dpi,
    )
    ...
    plot_balacc_vs_variation_near_fixed_metacells(
        ...
        out_png=os.path.join(
            near_fixed_dir,
            f"{dataset_name}_hvg_bal_acc_top10_near_{target_n_metacells}_metacells.png"
        ),
        dpi=dpi,
...
def analyze_one_partition_file(
    ...
    if len(chosen_gammas) == 0:
        logger.warning(
            "Skipping %s | hvg=%d | %s because no usable gamma columns found in %s",
    ...
    label_txt = os.path.join(
        label_dir,
        f"{dataset_name}_hvg_{variation_value}_{variant}_top{len(top_labels_fixed)}_labels_desc.txt"
    )
    ...
    for gamma in chosen_gammas:
        logger.info(
            "Running downstream | %s | hvg=%d | %s | gamma=%d | n_metacells=%d",
        ...
        out_base = os.path.join(
            ...
            f"{dataset_name}_hvg_{variation_value}_{variant}_gamma{gamma}_top10"
        )
    ...
    mean_out_base = os.path.join(
        heatmap_mean_dir,
        f"{dataset_name}_hvg_{variation_value}_{variant}_mean_near_{target_n_metacells}_top10"
    )
    plot_confusion_heatmap(
        cm=mean_cm,
...
def run_hvg_mode(
    ...
    logger.info("=" * 80)
    logger.info("Downstream mode: hvg")
    logger.info("=" * 80)

    mode_out_dir = os.path.join(output_root, dataset_name, "hvg", "downstream_celltypist")
    safe_mkdir(mode_out_dir)

    all_per_gamma = []
    all_aggregate = []

    for variation_value in hvg_values:
        try:
        ...
        logger.info(
            "Loading downstream original_adata | mode=hvg | value=%d | path=%s",
        ...
        if celltype_key not in original_adata.obs.columns:
            logger.warning(
                "Missing celltype key '%s' in %s, skipping hvg=%d.",
        ...
        for variant in variants:
            partition_csv = os.path.join(
                partition_dir,
                f"{dataset_name}_{variant}_hvg_{variation_value}_partitions.csv"
            )
    ...
    per_gamma_csv = os.path.join(summary_dir, f"{dataset_name}_hvg_top{top_k}_per_gamma_bal_acc.csv")
    aggregate_csv = os.path.join(summary_dir, f"{dataset_name}_hvg_top{top_k}_mean_bal_acc.csv")
    ...
    run_meta = {
        ...
        "preprocessed_dir": preprocessed_dir,
        "variation_values": [int(v) for v in hvg_values],
        "variants": variants,
        "target_n_metacells": int(target_n_metacells),
    ...
    write_metadata_json(
        os.path.join(mode_out_dir, f"{dataset_name}_hvg_downstream.meta.json"),
        run_meta,
    )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Human Fetal Atlas downstream CellTypist analysis, HVG only."
    )
    ...
    parser.add_argument("--hvg_preprocessed_dir", type=str, required=True)
...
def main():
    ...
    run_hvg_mode(
        ...
        preprocessed_dir=args.hvg_preprocessed_dir,
        output_root=args.output_root,
        hvg_values=args.hvg_values,
    ...
    logger.info("Human Fetal Atlas downstream CellTypist HVG-only analysis finished.")
