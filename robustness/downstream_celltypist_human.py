...
# Human fetal atlas gamma -> metacells mapping
GAMMA_TO_METACELLS = {
    20: 24729,
    30: 16486,
    40: 12364,
...
def resolve_preprocessed_h5ad(
    ...
    candidates = [
        os.path.join(preprocessed_dir, f"{dataset_name}_{variation_mode}_{variation_value}_prep.h5ad"),
        os.path.join(preprocessed_dir, f"{dataset_name}_camp1_{variation_mode}_{variation_value}_prep.h5ad"),
        os.path.join(preprocessed_dir, f"{dataset_name}_camp2_{variation_mode}_{variation_value}_prep.h5ad"),
        os.path.join(preprocessed_dir, f"{dataset_name}_camp3_{variation_mode}_{variation_value}_prep.h5ad"),
        os.path.join(preprocessed_dir, f"{dataset_name}_camp4_{variation_mode}_{variation_value}_prep.h5ad"),
    ]
    for p in candidates:
        if os.path.exists(p):
    ...
    raise FileNotFoundError(
        f"Could not find preprocessed h5ad for {dataset_name} {variation_mode}={variation_value} "
...
def create_condense_data(
    ...
    # remove fields that can make CellTypist think data is scaled
    cols_to_drop = [
        "highly_variable",
...
# =========================================================
# Line plots for downstream bal acc
# =========================================================
...
def make_lineplots_for_downstream(
    ...
    plot_balacc_overlay_all_values(
        ...
        variation_mode=variation_mode,
        out_png=os.path.join(overlay_dir, f"{dataset_name}_{variation_mode}_bal_acc_top10_overlay.png"),
        dpi=dpi,
    )
...
def run_one_mode(
    ...
    for variation_value in variation_values:
        ...
        logger.info(
            "Loading downstream original_adata | mode=%s | value=%d | path=%s",
            variation_mode, variation_value, original_adata_path
        )
        ...
        if celltype_key not in original_adata.obs.columns:
            logger.warning(
                "Missing celltype key '%s' in %s, skipping value=%d.",
        ...
        logger.info(
            "Top-%d labels | %s | %s=%d: %s",
            top_k, dataset_name, variation_mode, variation_value, top_labels_fixed
        )
    ...
    aggregate_csv = os.path.join(summary_dir, f"{dataset_name}_{variation_mode}_top{top_k}_mean_bal_acc.csv")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Human Fetal Atlas downstream CellTypist analysis using saved preprocessed h5ad and partition CSVs."
    )
...
def main():
    ...
    logger.info("Human Fetal Atlas downstream CellTypist analysis finished.")
