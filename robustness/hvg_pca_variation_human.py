...
def previous_ensure_gene_names(adata: AnnData, loom_var_name_key: str = "gene_short_name") -> AnnData:
    if loom_var_name_key in adata.var.columns:
        adata.var_names = adata.var[loom_var_name_key].astype(str)
        adata.var_names_make_unique()
        logger.info("Set var_names from %s", loom_var_name_key)
...
def build_or_load_base_cache(
    ...
    gene_means = np.asarray(adata.X.mean(axis=0)).ravel() if sparse.issparse(adata.X) else np.asarray(adata.X.mean(axis=0)).ravel()
...
def load_cache_with_sliced_pca(hvg_pca_h5ad: str, n_pcs: int) -> AnnData:
    ...
    logger.info("Loaded cache %s and sliced PCA to n_pcs=%d", hvg_pca_h5ad, n_pcs_use)
    return adata
...
# CAMP2/CAMP3 helpers
# =========================================================
def assign_from_cosine_to_seeds_on_the_fly(
    X_unit_all: np.ndarray,
    subset_idx: np.ndarray,
    seed_idx: np.ndarray,
    bs: int = 100_000,
):
    X_seeds = X_unit_all[seed_idx]
    parts = []
    for s in range(0, subset_idx.size, bs):
        e = min(s + bs, subset_idx.size)
        X_sub = X_unit_all[subset_idx[s:e]]
        sim_block = X_sub @ X_seeds.T
        parts.append(np.argmax(sim_block, axis=1))
    return np.concatenate(parts, axis=0)


def refine_once_cosine(
    adata: AnnData,
    subset_idx: np.ndarray,
    seed_idx: np.ndarray,
...
def assign_adaptive_gaussian_to_seeds_on_the_fly(
    ...
    for s in range(0, subset_idx.size, bs):
        ...
        W = np.exp(-(D_us ** 2) / denom)
        labels_parts.append(np.argmax(W, axis=1))

    return np.concatenate(labels_parts, axis=0)
...
def run_camp_variant(
    ...
    if variant in ("camp2", "camp3"):
        X_pca = adata.obsm["X_pca"].astype(np.float32, copy=False)
        adata.obsm["X_unit"] = normalize(X_pca, axis=1, copy=True)

    m_list, _ = create_m_list(adata, gamma_list, rep_key="X_pca")
    ...
    for gamma, m in zip(gamma_list, m_list):
        ...
        for anno in adata.obs[annotations].unique():
            ...
            if subset.n_obs <= min_metacells:
                ...
                scores = subset.obs["q"].values if "q" in subset.obs else compute_lightweight_coreset_q(subset, rep_key="X_pca")
                subset.obs["q"] = scores
                ...
                if variant == "camp1":
                    ...
                    labels_idx = assign_from_cosine_to_seeds_on_the_fly(
                        adata.obsm["X_unit"], subset_idx, seed_idx, bs=100_000
                    )
...
def compute_metrics_for_partition_file(
    ...
    for gamma_col in gamma_cols:
        labels = part[gamma_col].astype(str).values
        ad.obs["metacell"] = labels
...
def plot_metric_lines_single_variation(
    ...
    if variation_mode == "hvg":
        ...
        #title_suffix = f"n_hvg={variation_value}"
    else:
        title_suffix = f"PCs={variation_value}"
        #title_suffix = f"n_pcs={variation_value}"
...
def previous2_plot_metric_lines_single_gamma(
    summary_df: pd.DataFrame,
    dataset_name: str,
    variation_mode: str,
    metric_col: str,
    ...
    sub = summary_df[summary_df["gamma"] == gamma].copy()
    if sub.empty or metric_col not in sub.columns:
        logger.warning("Skipping %s gamma=%s", metric_col, gamma)
        return

    sub = sub.sort_values(["variation_value", "variant"])

    fig, ax = plt.subplots(figsize=(6.2, 4.8))

    for variant in [v for v in VARIANT_ORDER if v in set(sub["variant"])]:
        ss = sub[sub["variant"] == variant].sort_values("variation_value")
        ...
        #if std_col in ss.columns:
        #    y_std = ss[std_col].values.astype(float)
        #    ax.fill_between(
        #        x,
        #        y - y_std,
        #        y + y_std,
    ...
    ax.set_title(f"{dataset_name} | gamma={gamma}", fontsize=14)
    ax.set_xlabel("n_hvg" if variation_mode == "hvg" else "n_pcs", fontsize=12)
...
def previous_plot_metric_lines_single_gamma(
    summary_df: pd.DataFrame,
    dataset_name: str,
    variation_mode: str,
    metric_col: str,
    ...
    for variant in [v for v in VARIANT_ORDER if v in set(sub["variant"])]:
        ss = sub[sub["variant"] == variant].sort_values("variation_value")
        ax.plot(
            ss["variation_value"].values,
            ss[metric_col].values,
            marker="o",
            linewidth=2.2,
            markersize=5.5,
            color=variant_to_color(variant),
...
def previous_plot_ecdf_sc_ratio_single_gamma_variation(
    raw_df: pd.DataFrame,
    dataset_name: str,
    variation_mode: str,
    gamma: int,
    ...
    sub = raw_df[(raw_df["gamma"] == gamma) & (raw_df["variation_value"] == variation_value)].copy()
    if sub.empty or "sc_ratio" not in sub.columns:
        logger.warning("Skipping ECDF plot for gamma=%s value=%s", gamma, variation_value)
        return
    ...
    ax.set_title(f"{dataset_name} | gamma={gamma} | {'n_hvg' if variation_mode == 'hvg' else 'n_pcs'}={variation_value}", fontsize=14)
    ax.set_xlabel("sc_ratio", fontsize=12)
    ax.set_ylabel("ECDF", fontsize=12)
    ax.grid(False)
    ax.tick_params(labelsize=11)
...
def make_all_plots_for_dataset_variation(
    ...
    # line plots: one PNG per metric per gamma
    ...
    #    for gamma in gammas:
    #        out_png = os.path.join(metric_dir, f"{dataset_name}_{variation_mode}_{metric}_gamma{gamma}.png")
    #        plot_metric_lines_single_gamma(
    ...
    # ECDF plots of sc_ratio: one PNG per gamma per variation value
    #ecdf_dir = os.path.join(out_dir, "ecdf_sc_ratio")
    #safe_mkdir(ecdf_dir)

    #for gamma in gammas:
    #    gamma_dir = os.path.join(ecdf_dir, f"gamma_{gamma}")
    #    safe_mkdir(gamma_dir)
    #    for vv in variation_values:
    #        out_png = os.path.join(gamma_dir, f"{dataset_name}_{variation_mode}_ecdf_sc_ratio_gamma{gamma}_value{vv}.png")
    #        plot_ecdf_sc_ratio_single_gamma_variation(
    #            raw_df=raw_df,
    #            dataset_name=dataset_name,
    #            variation_mode=variation_mode,
    #            gamma=gamma,
    ...
    # ECDF plots of SC ratio: one PNG per variation value, for selected gammas only
    ecdf_dir = os.path.join(out_dir, "ecdf_sc_ratio")
    safe_mkdir(ecdf_dir)
...
def one_dataset_one_variation(
    ...
    for value in variation_values:
        ...
        prep_h5ad = os.path.join(prep_dir, f"{dataset_name}_{variation_mode}_{value}_prep.h5ad")
        if not os.path.exists(prep_h5ad):
            adata.write_h5ad(prep_h5ad, compression="gzip")

        prep_meta = os.path.join(prep_dir, f"{dataset_name}_{variation_mode}_{value}_prep.meta.json")
