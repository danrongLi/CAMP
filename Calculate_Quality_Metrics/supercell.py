...
print("initial adata shape:", covid_b.shape)

#metacell_partition = pd.read_csv(
#    "/storage/home/dvl5760/work/SEACells/customized_metacell/from_local_to_server_methods/human_supercell/supercell_membership_approx_new.csv",
#    index_col=0
#)
metacell_partition = pd.read_csv(
    "/storage/home/dvl5760/work/SEACells/customized_metacell/from_local_to_server_methods/human_supercell/supercell_membership_approx_new_large_metacell.csv",
    index_col=0
)

print("partition csv shape:", metacell_partition.shape)

covid_b.obs_names = covid_b.obs_names.astype(str)
metacell_partition.index = metacell_partition.index.astype(str)


common_idx = covid_b.obs_names.intersection(metacell_partition.index)
print("Common cells:", len(common_idx))


covid_b = covid_b[common_idx, :].copy()


print("Final adata shape:", covid_b.shape)
...
#metacell_partition = pd.read_csv(
#    "/storage/home/dvl5760/work/SEACells/customized_metacell/human_fetal_atlas/output/edit_4_add_ad_gau_partitions.csv",
#    index_col=0
#)

#metacell_partition = pd.read_csv(
#    "/storage/home/dvl5760/work/SEACells/customized_metacell/MetaQ/save/human_fetal_atlas_combined_metacell_labels.csv",
#    index_col=0
#)
...
#index_names = metacell_partition.index.tolist()
#print(index_names[:10]) 


#max_csv_index = metacell_partition.index.astype(int).max()
#max_adata_index = adata.obs_names.astype(int).max()

#print("Max index in CSV:", max_csv_index)
#print("Max index in adata.obs_names:", max_adata_index)

#print(adata.shape)
#print(metacell_partition.shape[0])
#print(X_diff_dense.shape[0])
...
m_list = metacell_partition.columns
#g_list = [103, 152, 201, 250, 300, 350, 400, 450, 500, 550]
#g_list = [550,500,450,400,350,300,250,200,150,100]
g_list = [70,60,50,40,30,20]
# ----------------------------
# Metrics helpers
# ----------------------------
...
for i in range(len(m_list)):
    ...
    covid_b.obs["metacell"] = metacell_partition[m_col].to_numpy()
...
out_path = os.path.join(out_dir, "supercell_metrics_human_fetal_atlas_large_metacell.csv")
