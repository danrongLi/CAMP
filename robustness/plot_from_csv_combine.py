...
def plot_metric_vs_variation_at_fixed_metacells(
    ...
    ax.grid(False)
    ax.tick_params(labelsize=24, width=2.0, length=6)
...
def main():
    ...
    # Ignore HVG=4000 for both summary and raw CSV inputs
    if args.variation_mode == "hvg":
        summary_df = summary_df[summary_df["variation_value"] != 4000].copy()
        raw_df = raw_df[raw_df["variation_value"] != 4000].copy()
